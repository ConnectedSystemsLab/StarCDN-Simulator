...
@desc
    This module implements the stationary orbit passes by assuming a 90 min re-entry period for every location.
    IMPORTANT: This model only works when using TimeBasedFOV module and the getLocation function is incorrect.
"""
import numpy as np
...
class ModelStationaryOrbit(IModel):
    '''
    ...
    def __get_Passes(self, **_kwargs) -> 'list[(Time, Time)]':
        ...
        #if the first entry is later than emulation start + 100min, return no passes
        if _cleanedList[0][0] >= _start.copy().add_seconds(100*60):
            return []
        ...
        reentry_out = []
        i = 0
        while _out[0][0].copy().add_seconds(i * 6000) <= _end:
            reentry_out.append((_out[0][0].copy().add_seconds(i * 6000), _out[0][1].copy().add_seconds(i * 6000)))
            if _out[0][1].copy().add_seconds(i * 6000) > _end:
                reentry_out[-1] = (reentry_out[-1][0], _end.copy())
                break
            i += 1
...
def init_ModelStationaryOrbit(
   ...
   @desc
    ...
    return ModelStationaryOrbit(_ownernodeins, _loggerins, _alwaysCalc)