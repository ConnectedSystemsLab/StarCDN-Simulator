...
class ModelOrbitNoMotion(IModel):
    '''
    ...
    def __get_Passes(self, **_kwargs) -> 'list[(Time, Time)]':
        ...
        # If starting time is included, assume it's always visible
        if _out[0][0] == _startTs:
            print(f"Can view {_startTs.copy(), _endTs.copy()}")
            return [_startTs.copy(), _endTs.copy()]
        else:
            return []
        return _out
    ...
    def __get_Position(self, **kwargs):
        ...
        if _time == self.__ownernode.simStartTime: 
            ...
            cur_loc = self.__ownernode.get_Position()
            self.__ownernode.update_Position(cur_loc, _time)
            return cur_loc
...
def init_ModelOrbitNoMotion(
    ...
    return ModelOrbitNoMotion(_ownernodeins, _loggerins, _alwaysCalc)